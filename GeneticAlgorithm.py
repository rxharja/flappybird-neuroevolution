...
class Population:
    ...
    def __init__(self, inp, pop_size=200):
        self.generation = 1
        self.members = []
        self.previous = []
            ...
            for itm in range(l):
                ...
                if (self.best_ten[i].alive < bird.alive and self.best_ten[i].pipes_passed < bird.pipes_passed):
                    if np.all(best_bool):
                        ...
                        if (self.best_ten[ind].alive < bird.alive and self.best_ten[ind].pipes_passed < bird.pipes_passed):
    ...
    def select(self, weight=0.99):
        ...
        for bird in self.best_ten:
            if bird.alive*bird.pipes_passed > 0:
                ...
                new_pop.append(Bird(230,0,NeuralNet(1, 4, [0, 0, 0, 0])))
